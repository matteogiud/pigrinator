...
class MyBoxLayout(BoxLayout):
    ...
    def send_post_requestAcqua(self, *args):

        url = "http://pigrinatorstand.local/loadWater"  # Sostituisci con l'URL corretto per la richiesta POST

        def on_success(request, result):
            self.result_label.text = "Sto riempiendo!"

        def on_failure(request, result):
            self.result_label.text = "Errore nella richiesta POST"

        UrlRequest(
            url, on_success=on_success, on_failure=on_failure
        )    
